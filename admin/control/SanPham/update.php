		...
		<div id="them">			
				<form method="POST" enctype="multipart/form-data">
					...
					<div class="form-row">
						...
						<div class="form-group col-md-6">
							<label >Khuyến mãi <i>(1:ĐT ,2:Laptop, 3:PC)</i></label>
							<select id="inputState" name="KhuyenMai" class="form-control">
								<?php 
								$qr = mysqli_query($connect,"SELECT * from khuyenmai");
								while ($row2 = mysqli_fetch_array($qr)) {
									if($row1['idKhuyenMai']==$row2['idKhuyenMai']){
								?>
										<option selected><?= $row2['idKhuyenMai']?></option>
									<?php }else{?>
										<option><?= $row2['idKhuyenMai']?></option>
									<?php } ?>
							<?php } ?>
							</select>
						</div>
